...
function purchaseClicked() {
    alert('شكراً لك على تعاملك معنا')
...
function addItemToCart(title, price, imageSrc) {
    ...
    for (var i = 0; i < cartItemNames.length; i++) {
        if (cartItemNames[i].innerText == title) {
            alert('لقد قمت بالفعل بتحديد هذا العنصر')
            return
        }
    }
    var cartRowContents = `
    <div class="cart-item cart-column">
    <img class="shop-item-image" src="${imageSrc}" width="50%" height="auto">
    </div>
...
function updateCartTotal() {
    ...
    document.getElementsByClassName('cart-total-price')[0].innerText = total + ' MRO'
}